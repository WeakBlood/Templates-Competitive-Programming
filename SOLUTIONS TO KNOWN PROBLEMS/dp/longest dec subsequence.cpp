...
int longest_decreasing_subsequence(const std::vector<int>& arr) 
{
    ...
    for (int i = 1; i < n; ++i) 
    {
        ...
        /*
            decreasing or equal:
            auto it = std::upper_bound(tail.begin(), tail.begin() + len, arr[i],std::greater<int>());
            increasing:
            auto it = std::lower_bound(tail.begin(), tail.begin() + len, arr[i]);
            increasing or equal:
            auto it = std::upper_bound(tail.begin(), tail.begin() + len, arr[i]);
        */
